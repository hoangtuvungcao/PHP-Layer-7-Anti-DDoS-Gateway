...
class SessionHardeningService
{
    ...
    public function enforce(SecurityContext $context): SecurityResult
    {
        ...
        $state['tab_tokens'][$lockToken] = time();
        ...
        return SecurityResult::passThrough([
            'layer' => 5,
            'session_lock' => $lockToken,
            'tabs' => count($state['tab_tokens']),
    ...
    private function verifyFingerprint(SecurityContext $context, array &$state)
    {
        ...
        return SecurityResult::challenge('Fingerprint mismatch detected', $this->buildMetadata($state, 'fingerprint_mismatch'));
    }
    ...
    private function verifyUserAgent(SecurityContext $context, array &$state)
    {
        ...
        return SecurityResult::challenge('User agent mismatch', $this->buildMetadata($state, 'ua_mismatch'));
    }
    ...
    private function verifyIp(SecurityContext $context, array &$state)
    {
        ...
        return SecurityResult::challenge('IP mismatch', $this->buildMetadata($state, 'ip_mismatch'));
    }
    ...
    private function enforceTabLimits(SecurityContext $context, array &$state)
    {
        ...
        $activeTabs = count($state['tab_tokens']);
        if ($activeTabs <= $maxTabs) {
            return null;
        }

        $context->addNote('layer5:too-many-tabs');
        return SecurityResult::challenge('Too many concurrent tabs', $this->buildMetadata($state, 'too_many_tabs', [
            'tabs' => $activeTabs,
            'max_tabs' => $maxTabs,
        ]));
    }
    ...
    private function refreshGuard(SecurityContext $context, array &$state)
    {
        ...
        $metadata = $this->buildMetadata($state, 'refresh_overflow', [
    ...
    private function ensureLockCookie(array &$state): string
    {
        ...
        if (!$token || !isset($state['tab_tokens'][$token])) {
    ...
    private function buildMetadata(array $state, ?string $reason = null, array $extra = []): array
    {
        $metadata = [
            'layer' => 5,
            'reason' => $reason,
            'session_lock' => $state['lock_token'] ?? null,
            'tabs' => isset($state['tab_tokens']) ? count($state['tab_tokens']) : 0,
            'max_tabs' => (int) ($this->config['max_tabs'] ?? 0),
            'refresh_count' => $state['refresh']['count'] ?? 0,
            'refresh_window_start' => $state['refresh']['window_start'] ?? null,
            'refresh_last' => $state['refresh']['last_time'] ?? null,
