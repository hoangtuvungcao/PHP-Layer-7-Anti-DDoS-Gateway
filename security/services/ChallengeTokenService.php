...
class ChallengeTokenService
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->lockCookieName = (string) ($config['lock_cookie_name'] ?? '__sec_ch_lock');
    }
    ...
    public function getLockCookieTimestamp(): ?int
    {
        if (!isset($_COOKIE[$this->lockCookieName])) {
            return null;
        }

        $value = (int) $_COOKIE[$this->lockCookieName];
        return $value > 0 ? $value : null;
    }

    public function issueLockCookie(int $lockedUntil): void
    {
        if ($lockedUntil <= time()) {
            $this->clearLockCookie();
            return;
        }

        setcookie($this->lockCookieName, (string) $lockedUntil, [
            'expires' => $lockedUntil,
            'path' => $this->cookiePath,
            'secure' => $this->cookieSecure,
            'httponly' => false,
    ...
    public function clearLockCookie(): void
    {
        setcookie($this->lockCookieName, '', [
            'expires' => time() - 3600,
            'path' => $this->cookiePath,
            'secure' => $this->cookieSecure,
            'httponly' => false,
